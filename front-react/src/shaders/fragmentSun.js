...
const fragmentSunShader = `
    ...
    {
        n *= 0.25;
        return (vec3(n, n*n, n*n*n*n)/0.25)*0.6;
    }

    void main() {
        // Calculate 4D noise
        vec4 p = vec4(vPosition * 0.005, time * 0.05);
        float noise4d = fbm(p);

        // Calculate spots based on 4D noise
        vec4 p1 = vec4(vPosition * 0.15, time * 0.05);
        float spots = max(snoise(p1), 0.0);

        // Convert the noise value to a color
        vec3 sunColor = brightness2Color(noise4d);
        sunColor = sunColor * 10.0 * vec3(1.2, 0.8, 0.5) + vec3(0.1, 0.05, 0.0);
